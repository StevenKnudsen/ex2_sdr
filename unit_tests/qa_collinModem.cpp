...
TEST(collinModem, FileReadTest )
{
    /* ---------------------------------------------------------------------
    * Confirm that a 16 bit binary file can be read
    * ---------------------------------------------------------------------
    */

    // streampos size;
    int size = 440000*2; // Gnuradio imports 4.4e6 ishorts Need to resample.
    // int size = 12;
    char * memblock;

    std::complex<float> float_data [size/4];   // frame samples (received)

    ifstream file ("../../../usrp_rec_50k.dat", ios::in|ios::binary);
    if (file.is_open()) {
        // size = file.tellg();
        ...
        for(int i = 0; i < size; i += 4) {
            uint16_t real = (memblock[i] << 8) + (memblock[i+1]);
            uint16_t imag = (memblock[i+2] << 8) + (memblock[i+3]);
            std::complex<float> float_sample(real, imag);
            float_data[i/4] = float_sample;
        }
        // cout << float_data;
        // cout << "\n";
    } else { 
    ...
    unsigned int k           =   20;     // filter samples/symbol
